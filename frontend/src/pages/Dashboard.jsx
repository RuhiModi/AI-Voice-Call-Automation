...
function StatCard({ label, value, icon: Icon, color, change, delay }) {
  const colors = {
    indigo: 'from-navy-500 to-navy-600',
    orange: 'from-orange-400 to-orange-500',
    green: 'from-jade-400 to-jade-600',
    purple: 'from-purple-400 to-purple-600',
  }
  return (
    <div className="bg-white rounded-2xl p-6 border border-navy-100/60 card-hover animate-slide-up"
      ...
      <div className="bg-white rounded-3xl border border-navy-100/60 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-navy-400 text-sm mb-6">Create your first AI voice campaign</p>
            <Link to="/dashboard/campaigns/new"
            ...
            {campaigns.slice(0, 5).map((c, i) => {
              ...
              return (
                <Link key={c.id} to={`/dashboard/campaigns/${c.id}`}
                  className="flex items-center gap-5 px-7 py-5 hover:bg-navy-50/50 transition-colors group animate-slide-up"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex-1 h-1.5 bg-navy-100 rounded-full overflow-hidden max-w-[140px]">
                        <div className="h-full bg-gradient-to-r from-navy-500 to-orange-400 rounded-full transition-all"
                          style={{ width: `${pct}%` }} />
                      </div>
                      <span className="text-xs text-navy-400">{done}/{total}</span>
                    </div>
                  </div>
                  {/* Status */}
