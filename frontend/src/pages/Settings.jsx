...
import {
  Phone, Key, Sheet, Bell, Shield, ChevronRight,
  Check, ExternalLink, AlertCircle, Globe, Mic2
} from 'lucide-react'
  ...
  return (
    <div className="p-6 lg:p-10 max-w-3xl mx-auto">
      ...
      <Section title="Profile" desc="Your account information" icon={Shield}>
        <InputField label="Company Name" value={profile.company_name}
          onChange={v => setProfile(p => ({ ...p, company_name: v }))}
          placeholder="Your company name" />
        <InputField label="Email" value={profile.email} type="email"
          onChange={v => setProfile(p => ({ ...p, email: v }))}
          placeholder="you@company.com"
          hint="Email cannot be changed after signup" />
        <button onClick={() => save('Profile')}
          className="px-5 py-2.5 bg-ink-900 text-white rounded-2xl text-sm font-semibold hover:bg-ink-800 transition-colors">
          Save Profile
        </button>
      </Section>
      ...
      <Section title="Vobiz Telephony" desc="Primary telephony provider for India calls" icon={Phone}>
        <div className="flex items-center gap-2 mb-5 p-3 bg-saffron-50 border border-saffron-200 rounded-2xl">
          ...
          <p className="text-xs text-saffron-700">
            ...
            <a href="https://vobiz.in" target="_blank" rel="noopener noreferrer"
              className="font-semibold underline hover:text-saffron-900">vobiz.in</a>
            . Contact their support for faster activation.
          </p>
        </div>
        <InputField label="Vobiz API Key" value={vobiz.api_key} type="password"
          onChange={v => setVobiz(p => ({ ...p, api_key: v }))}
          placeholder="Your Vobiz API key" />
        <InputField label="API URL" value={vobiz.api_url}
          onChange={v => setVobiz(p => ({ ...p, api_url: v }))}
          placeholder="https://api.vobiz.in/v1" />
        <InputField label="Default Caller ID" value={vobiz.default_caller_id}
          onChange={v => setVobiz(p => ({ ...p, default_caller_id: v }))}
          placeholder="+917900000001"
          hint="Must be a registered number with Vobiz" />
        <button onClick={() => save('Vobiz')}
          className="px-5 py-2.5 bg-ink-900 text-white rounded-2xl text-sm font-semibold hover:bg-ink-800 transition-colors">
          Save Vobiz Settings
        </button>
      </Section>
      ...
      {/* Exotel */}
      <Section title="Exotel Telephony" desc="Backup telephony provider (PSTN)" icon={Phone}>
        <div className="flex items-center gap-2 mb-5 p-3 bg-ink-50 border border-ink-100 rounded-2xl">
          ...
          <p className="text-xs text-ink-500">
            Exotel free trial gives test credits. Sign up at{' '}
            <a href="https://exotel.com" target="_blank" rel="noopener noreferrer"
              className="font-semibold underline hover:text-ink-700">exotel.com</a>
          </p>
        </div>
        <InputField label="Exotel SID" value={exotel.sid}
          onChange={v => setExotel(p => ({ ...p, sid: v }))}
          placeholder="Your Exotel account SID" />
        <InputField label="Exotel Token" value={exotel.token} type="password"
          onChange={v => setExotel(p => ({ ...p, token: v }))}
          placeholder="Your Exotel auth token" />
        <InputField label="Subdomain" value={exotel.subdomain}
          onChange={v => setExotel(p => ({ ...p, subdomain: v }))}
          placeholder="yourcompany.exotel.com" />
        <button onClick={() => save('Exotel')}
          className="px-5 py-2.5 bg-ink-900 text-white rounded-2xl text-sm font-semibold hover:bg-ink-800 transition-colors">
      ...
      {/* OpenAI */}
      <Section title="OpenAI (AI Brain)" desc="GPT-4o mini powers all conversations" icon={Mic2}>
        <div className="flex items-center gap-2 mb-5 p-3 bg-jade-50 border border-jade-200 rounded-2xl">
          ...
          <p className="text-xs text-jade-700">
            Using <strong>gpt-4o-mini</strong> — best cost/quality ratio. ~₹0.04 per call.
            Get key from <a href="https://platform.openai.com" target="_blank" rel="noopener noreferrer"
              className="font-semibold underline">platform.openai.com</a>
          </p>
        </div>
        <InputField label="OpenAI API Key" value={openai.api_key} type="password"
          onChange={v => setOpenai(p => ({ ...p, api_key: v }))}
          placeholder="sk-proj-..." />
        <div className="mb-4">
          <label className="block text-xs font-bold text-ink-500 uppercase tracking-wider mb-1.5">Model</label>
          <select value={openai.model} onChange={e => setOpenai(p => ({ ...p, model: e.target.value }))}
            className="w-full bg-ink-50 border border-ink-100 rounded-2xl px-4 py-3 text-sm text-ink-800 focus:border-ink-300">
            <option value="gpt-4o-mini">gpt-4o-mini (Recommended — cheapest)</option>
            <option value="gpt-4o">gpt-4o (More capable, 10x cost)</option>
          </select>
        </div>
        <button onClick={() => save('OpenAI')}
          className="px-5 py-2.5 bg-ink-900 text-white rounded-2xl text-sm font-semibold hover:bg-ink-800 transition-colors">
      ...
      {/* Google Cloud */}
      <Section title="Google Cloud" desc="STT + TTS for Gujarati, Hindi, English" icon={Mic2}>
        <div className="flex items-center gap-2 mb-5 p-3 bg-blue-50 border border-blue-200 rounded-2xl">
          ...
          <p className="text-xs text-blue-700">
            Enable <strong>Speech-to-Text</strong> and <strong>Text-to-Speech</strong> APIs in Google Cloud Console.
            New accounts get $300 free credit.
          </p>
        </div>
        <InputField label="Google Cloud Project ID" value={google.project_id}
          onChange={v => setGoogle(p => ({ ...p, project_id: v }))}
          placeholder="ai-voice-saas-123456"
          hint="Found in Google Cloud Console dashboard" />
        <div className="mb-4">
          ...
          <div className="flex items-center gap-3 border-2 border-dashed border-ink-200 rounded-2xl p-4">
            ...
            <a href="https://console.cloud.google.com" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1 text-xs text-ink-500 hover:text-ink-800 font-medium whitespace-nowrap">
              Open Console <ExternalLink size={11} />
            </a>
          </div>
        </div>
        <button onClick={() => save('Google Cloud')}
          className="px-5 py-2.5 bg-ink-900 text-white rounded-2xl text-sm font-semibold hover:bg-ink-800 transition-colors">
      ...
      {/* Google Sheets */}
      <Section title="Google Sheets" desc="Auto-save call results to your spreadsheet" icon={Sheet || Key}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-ink-800">
                {google.sheets_connected ? 'Connected' : 'Not Connected'}
              </p>
              <p className="text-xs text-ink-400 mt-0.5">
                {google.sheets_connected ? 'Call results will auto-sync' : 'Connect to sync call results'}
              </p>
            </div>
          </div>
          <button onClick={connectSheets}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all
              ${google.sheets_connected
        ...
        <p className="text-xs text-ink-400">
          Each completed call automatically adds a row: Phone, Name, Outcome, Language, Duration, Collected Data, Timestamp
        </p>
      </Section>
            ...
            <div key={key} className="flex items-center justify-between py-3 border-b border-ink-50 last:border-0">
              ...
              <button onClick={() => setNotifications(n => ({ ...n, [key]: !n[key] }))}
                className={`relative w-11 h-6 rounded-full transition-all duration-200
                  ${notifications[key] ? 'bg-ink-800' : 'bg-ink-200'}`}>
      ...
      {/* Cost info */}
      <div className="bg-gradient-to-br from-ink-900 to-ink-800 rounded-3xl p-6 border border-ink-700 animate-slide-up">
        <h3 className="font-display font-bold text-white mb-4">💰 Cost Calculator</h3>
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="col-span-2 flex justify-between items-center pt-2">
            <span className="text-white font-semibold">Total per call</span>
            <span className="text-saffron-400 font-display font-bold text-lg">~₹1.05</span>
          </div>
        </div>
      </div>
    </div>
...
// icon fix
function Sheet(props) { return <Key {...props} /> }
function Mic2(props) { return <Mic2Icon {...props} /> }
function Mic2Icon({ size, className }) {
  return (
