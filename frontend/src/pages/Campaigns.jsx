...
import {
  Plus, Search, Phone, ChevronRight, Filter,
  Megaphone, Clock, CheckCircle, PauseCircle, FileEdit
} from 'lucide-react'
...
const statusMeta = {
  active: { label: 'Active', dot: 'bg-jade-400', text: 'text-jade-700', bg: 'bg-jade-50', icon: Clock, pulse: true },
  completed: { label: 'Completed', dot: 'bg-navy-400', text: 'text-navy-600', bg: 'bg-navy-50', icon: CheckCircle, pulse: false },
  paused: { label: 'Paused', dot: 'bg-orange-400', text: 'text-orange-700', bg: 'bg-orange-50', icon: PauseCircle, pulse: false },
  draft: { label: 'Draft', dot: 'bg-gray-300', text: 'text-gray-500', bg: 'bg-gray-50', icon: FileEdit, pulse: false },
}
...
export default function Campaigns() {
  ...
  useEffect(() => {
    if (DEMO) { setCampaigns(mockCampaigns); setLoading(false); return }
    campaignApi.list()
    ...
    <div className="p-6 lg:p-10 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 animate-fade-in">
        <div>
          ...
          <p className="text-navy-400 text-sm mt-1">{campaigns.length} total · {counts.active} active</p>
        </div>
        <Link to="/dashboard/campaigns/new"
          className="inline-flex items-center gap-2 bg-navy-900 hover:bg-navy-800 text-white px-5 py-3 rounded-2xl font-semibold text-sm transition-all hover:shadow-lg hover:shadow-navy-200 group">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6 animate-slide-up">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="w-full bg-white border border-navy-100 rounded-2xl pl-10 pr-4 py-3 text-sm text-navy-800 placeholder-navy-300 transition-all focus:border-navy-300" />
        </div>
        <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)}
          className="bg-white border border-navy-100 rounded-2xl px-4 py-3 text-sm text-navy-700 cursor-pointer focus:border-navy-300 transition-all">
          {TYPE_FILTERS.map(t => (
      ...
      {/* Status tabs */}
      <div className="flex gap-2 mb-6 overflow-x-auto pb-1 animate-slide-up">
        {STATUS_FILTERS.map(s => (
          <button key={s} onClick={() => setStatusFilter(s)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all
              ${statusFilter === s
                ? 'bg-navy-900 text-white shadow-sm'
                : 'bg-white border border-navy-100 text-navy-500 hover:border-navy-200 hover:text-navy-700'}`}>
            {s === 'all' ? 'All' : s.charAt(0).toUpperCase() + s.slice(1)}
            <span className={`text-xs px-1.5 py-0.5 rounded-lg font-semibold
              ${statusFilter === s ? 'bg-white/20 text-white' : 'bg-navy-50 text-navy-500'}`}>
              {counts[s]}
            </span>
          </button>
        ))}
              ...
              <Link key={c.id} to={`/dashboard/campaigns/${c.id}`}
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[11px] font-bold px-2 py-1 rounded-lg border ${langColor[c.language_priority]}`}>
                ...
                {c.description && (
                  <p className="text-navy-400 text-xs mb-4 line-clamp-1">{c.description}</p>
                )}
                ...
                <div className="mt-4">
                  ...
                  <div className="h-1.5 bg-navy-50 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-navy-500 to-orange-400 rounded-full transition-all duration-700"
                      style={{ width: `${pct}%` }} />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 mt-4 pt-4 border-t border-navy-50">
                  ...
                  <span className="text-xs text-navy-400 capitalize">{c.campaign_type}</span>
                </div>
              </Link>
            )
          })}
