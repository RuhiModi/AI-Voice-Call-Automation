  ...
  return (
    <div className="p-6 lg:p-10 max-w-3xl mx-auto animate-fade-in">
      ...
      <div className="mb-8">
        ...
        <p className="text-navy-400 text-sm">Set up your AI voice calling campaign</p>
        {campaignId && (
          <p className="text-xs text-jade-600 mt-1">✅ Saved to database · ID: {campaignId.slice(0,8)}...</p>
        )}
      </div>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-2 flex-shrink-0">
                ...
                {i < STEPS.length - 1 && <ChevronRight size={14} className={done ? 'text-jade-400' : 'text-navy-200'} />}
              </div>
            )
          })}
        </div>
        <div className="h-1.5 bg-navy-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-navy-500 to-orange-400 rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-navy-100 p-7 mb-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <textarea value={form.description} onChange={e => set('description', e.target.value)}
                ...
                className="w-full bg-navy-50 border border-navy-100 rounded-2xl px-4 py-3 text-sm text-navy-800 placeholder-navy-300 focus:border-navy-400 focus:outline-none resize-none transition-all" />
            </div>
            <div>
              <label className="block text-xs font-bold text-navy-500 uppercase tracking-wider mb-3">Primary Language</label>
              <div className="grid grid-cols-3 gap-3">
                {LANG_OPTIONS.map(l => (
                  <button key={l.code} onClick={() => set('language_priority', l.code)}
                    className={`flex flex-col items-center p-4 rounded-2xl border-2 transition-all
                      ${form.language_priority === l.code ? 'border-navy-600 bg-navy-50 shadow-sm' : 'border-navy-100 hover:border-navy-200'}`}>
                ...
                {CAMPAIGN_TYPES.map(t => (
                  <button key={t.type} onClick={() => set('campaign_type', t.type)}
                    className={`flex items-start gap-3 p-4 rounded-2xl border-2 text-left transition-all
                      ${form.campaign_type === t.type ? 'border-navy-600 bg-navy-50' : 'border-navy-100 hover:border-navy-200'}`}>
          ...
          <div className="space-y-6">
                ...
                {[
                  ...
                  <button key={s.type} onClick={() => set('script_type', s.type)}
                    className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all
                      ${form.script_type === s.type ? 'border-navy-600 bg-navy-50' : 'border-navy-100 hover:border-navy-200'}`}>
              ...
              {form.script_type === 'url' && (
                <div className="flex gap-3 mb-4">
                  <input value={urlInput} onChange={e => setUrlInput(e.target.value)}
                    ...
                    className="flex-1 bg-navy-50 border border-navy-100 rounded-2xl px-4 py-3 text-sm text-navy-800 placeholder-navy-300 focus:border-navy-400 focus:outline-none" />
            ...
            <div>
              ...
              <textarea value={form.script_content} onChange={e => set('script_content', e.target.value)}
                ...
                className="w-full bg-navy-50 border border-navy-100 rounded-2xl px-4 py-3 text-sm text-navy-700 placeholder-navy-300 font-mono leading-relaxed resize-none focus:border-navy-400 focus:outline-none transition-all" />
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input value={form.persona_name} onChange={e => set('persona_name', e.target.value)}
                  placeholder="Priya"
                  className="w-full bg-navy-50 border border-navy-100 rounded-2xl px-4 py-3 text-sm text-navy-800 placeholder-navy-300 focus:border-navy-400 focus:outline-none" />
            ...
            <div>
              ...
              <div className="flex gap-2 mb-3">
                ...
                <button onClick={() => { if (fieldInput.trim()) { set('data_fields', [...form.data_fields, fieldInput.trim()]); setFieldInput('') }}}
                  className="w-11 h-11 bg-navy-900 text-white rounded-2xl flex items-center justify-center hover:bg-navy-800 flex-shrink-0">
                  <Plus size={16} />
                </button>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                {form.data_fields.length === 0 && <p className="text-xs text-navy-300 italic">Press Enter to add a field</p>}
              </div>
            </div>
            ...
            <div className="p-4 bg-navy-50 rounded-2xl border border-navy-100">
              ...
              {['✅ Auto-detect language (gu/hi/en)', '✅ Reschedule if user is busy', '✅ Transfer to human on request', '✅ Mark DNC if user says "stop calling"'].map(b => (
                <p key={b} className="text-xs text-navy-500 mb-1">{b}</p>
              ))}
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div className="p-4 bg-orange-50 border border-orange-200 rounded-2xl">
              ...
              <p className="text-xs text-orange-700 mt-2">Column names must match script variables e.g. {`{{name}}`} → column "name"</p>
            </div>
            ...
            <div>
              ...
              {contactCount > 0 && (
                <div className="mt-3 flex items-center gap-2 p-3 bg-jade-50 border border-jade-200 rounded-xl">
                  ...
                  <span className="text-sm font-semibold text-jade-700">{contactCount} contacts imported to database!</span>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              {form.google_sheet_id && (
                <p className="text-xs text-jade-600 mt-1">✅ Sheet ID: {form.google_sheet_id.substring(0, 20)}...</p>
              )}
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-6">
            ...
            {contactCount === 0 && (
              <div className="p-4 bg-orange-50 border border-orange-200 rounded-2xl">
                <p className="text-xs text-orange-700 font-semibold">⚠️ No contacts uploaded yet</p>
                <p className="text-xs text-orange-600 mt-1">Go back to Step 3 and upload a CSV file. Campaign will launch but won't call anyone without contacts.</p>
              </div>
            )}
          </div>
        )}
